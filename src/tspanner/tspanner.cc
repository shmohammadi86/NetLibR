...
// From: "Detecting Community Structures in Social Networks by Graph Sparsification"
// https://github.com/satyakisikdar/spanner-comm-detection
...
namespace NetLibR {
	...
	arma::sp_mat make_spanner(arma::sp_mat &G_adj, int k) {
		...
		// constructs the (2k - 1)-spanner using Baswana and Sen's randomized algorithm 
		Graph G, G_comm, G_prime, G_spanner;
			...
			for(int v : unsampled_nodes)
			{
				...
				if(neighborhood[v] == -1) //v is not adjacent to any sampled nodes
				{
					...
					for(auto item : min_table)
					{
						...
						G_sparse(v, item.second.second) = item.second.first;
...
/*
		...
		IndexMap index = get(vertex_index, G_prime);
		
		graph_traits<Graph>::edge_iterator ei, ei_end;
		for (tie(ei, ei_end) = edges(G_prime); ei != ei_end; ++ei) {
			int u = index[source(*ei, G_prime)];
			int v = index[target(*ei, G_prime)];
